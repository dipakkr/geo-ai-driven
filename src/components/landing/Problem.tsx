...
const pastelColors = ["bg-[#DBF6FF]", "bg-[#E0FFD4]", "bg-[#FFE7F8]"];
  ...
  return (
    <section ref={ref} className="px-6 py-24 md:py-[100px]">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mb-14 max-w-4xl mx-auto rounded-3xl bg-white border border-black/[0.06] p-8 sm:p-12 shadow-[0_2px_20px_rgba(0,0,0,0.06)]">
          <div className="flex flex-col gap-6 text-base text-[#6f6f6f] leading-relaxed font-light relative">
            <p>
              A small business owner needs liability coverage for her new warehouse. She doesn't Google it. She opens ChatGPT and types:
            </p>
            <div className="relative rounded-2xl bg-[#f5f5f5] p-5 sm:p-6 text-[#0a0a0a] font-medium italic">
               <div className="absolute -top-3 -left-3 rounded-full bg-[#1483F3] p-2 text-white shadow-sm">
            ...
            <p className="font-medium text-[#0a0a0a] py-2 px-4 border-l-2 border-[#1483F3] bg-[#f5f5f5] rounded-r-lg">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {cards.map((card, i) => (
            <div
              key={card.title}
              className={`rounded-3xl ${pastelColors[i]} p-8 card-hover min-h-[220px] transition-all duration-500 ${
                isVisible ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
              }`}
              style={{ transitionDelay: `${i * 150}ms` }}
            >
              <div className="mb-4 text-3xl">{card.icon}</div>
              <h3 className="mb-2 text-lg font-medium text-[#0a0a0a]">{card.title}</h3>
              <p className="text-sm text-[#6f6f6f] leading-relaxed">{card.body}</p>
            </div>
          ))}
        </div>
      </div>
